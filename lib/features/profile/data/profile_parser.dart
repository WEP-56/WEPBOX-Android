...
class ProfileParser {
  ...
  TaskEither<ProfileFailure, ProfileEntriesCompanion> addLocal({
    ...
    return TaskEither.tryCatch(() async {
        ...
        .flatMap(
          (_) => TaskEither.fromEither(populateHeaders(content: content)),
        )
        .flatMap(
      ...
      final rs = await _httpClient
          .download(
            ...
            userAgent: _ref.read(ConfigOptions.useXrayCoreWhenPossible)
                ? _httpClient.userAgent.replaceAll("WEPBOX", "WEPBOXX")
                : null,
          )
          .catchError((err) {
        ...
        try {
          ...
          await httpClient.download(
            ...
            userAgent: ref.read(ConfigOptions.useXrayCoreWhenPossible)
                ? httpClient.userAgent.replaceAll('WEPBOX', 'WEPBOXX')
                : null,
          );
  ...
  static SubscriptionInfo? _parseSubscriptionInfo(String subInfoStr) {
    ...
    final map = {
      for (final v in values)
        v.split('=').first.trim(): num.tryParse(
          v.split('=').second.trim(),
        )?.toInt(),
    };
    if (map case {
