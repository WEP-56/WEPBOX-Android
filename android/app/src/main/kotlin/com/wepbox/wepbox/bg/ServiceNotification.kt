...
import com.wepbox.wepbox.R
import com.wepbox.wepbox.Settings
import com.wepbox.wepbox.constant.Action
import com.wepbox.wepbox.constant.Status
//import com.wepbox.wepbox.utils.CommandClient
import com.hiddify.core.libbox.Libbox
import com.wepbox.wepbox.Application.Companion.notification
import com.wepbox.wepbox.utils.GrpcClientProvider
    ...
    fun show(profileName: String, @StringRes contentTextId: Int) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            Application.notification.createNotificationChannel(
                NotificationChannel(
                    notificationChannel, "WEPBOX service", NotificationManager.IMPORTANCE_LOW
                )
            )
        }
        service.startForeground(
            notificationId, notificationBuilder
                .setContentTitle(profileName.takeIf { it.isNotBlank() } ?: "WEPBOX")
...
//            commandClient.connect()
    ...
    fun updateStatus(previous:SystemInfo,status: SystemInfo) {
        ...
        val content = "${Libbox.formatBytes(uplink)}/s 鈫慭t${Libbox.formatBytes(downlink)}/s 鈫?\n${status.current_outbound}"
    ...
    fun startListenSystemInfo() {
        ...
        streamingJob = streamingCoroutineScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                while (isActive) {
                    delay(1_000) // 鉁?coroutine-friendly
